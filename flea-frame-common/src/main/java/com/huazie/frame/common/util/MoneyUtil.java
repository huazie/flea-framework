...
public class MoneyUtil {
    ...
    private final static String MONEY_FORMAT_PATTERN = "###,##0.00";
    ...
    public static String getCentToDollarInt(long cent) {
        ...
        String dollar = m.toString();

        return dollar.substring(0, dollar.indexOf("."));
    }

    /**
    * 根据单位为分的金额，获取格式化后的金钱（每三位一个逗号）
    *   如12345678，返回123,456.78
    * @param cent  12345678
    * @return 123,456.78
    */
    public static String getCentToFormat(long cent) {
        Money m = new Money(cent);
        DecimalFormat decimalFormat = new DecimalFormat(MONEY_FORMAT_PATTERN);

        return decimalFormat.format(Double.parseDouble(m.toString()));
    }

    /**
     * 获取元到分
     * @param dollar 元  11.00
     * @return 1100 分
     */
...
class Money implements Serializable, Comparable {
    ...
    */
    ...
    /*  public String getDisplayUnit(HttpServletRequest request) {
          return getDisplayUnit(request.getLocale());
      }*/
