...
public class StringUtils {
	 ...
	 *            替换字符串
	 */
	public static void replace(StringBuilder strBuilder, String placeholder, String value) throws Exception {
		int start = strBuilder.indexOf(placeholder);
		if (start == -1) {
			throw new Exception("请检查占位符placeholder=【" + placeholder + "】是否存在");
		}
	...
	public static String[] split(String value, String... placeholders) {
		...
		for (int i = 0; i < placeholders.length; i++) {
			if (i < placeholders.length - 1) {
				phs.append(placeholders[i]).append(CommonConstants.SymbolConstants.VERTICALLINE);
			} else {
