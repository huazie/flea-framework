 ...
 * @since 2.0.0
 */
public class SelectSqlTemplateStrategy<T> implements IFleaStrategy<SqlTemplate<T>, SqlTemplatePOJO<T>> {

    @Override
    public SqlTemplate<T> execute(SqlTemplatePOJO<T> contextParam) throws FleaStrategyException {
        return new SelectSqlTemplate<>(contextParam.getRelationId(), contextParam.getEntity());
    }
}
