...
public class InsertSqlTemplate<T> extends SqlTemplate<T> {
    ...
    protected void initSqlTemplate(StringBuilder sql, Map<String, Object> params, final Column[] entityCols, Map<String, Property> propMap) throws CommonException {
        ...
        if (ObjectUtils.isEmpty(columns) && ObjectUtils.isEmpty(values)) { // 表示将实体类的全部变量替换
            ...
            valStr = StringUtils.strCombined(entityCols, Column.COLUMN_ATTR_NAME, DBConstants.SQLConstants.SQL_BLANK + DBConstants.SQLConstants.SQL_COLON, DBConstants.SQLConstants.SQL_COLON, DBConstants.SQLConstants.SQL_COMMA);
    ...
    private Column[] check(final Column[] entityCols, String[] cols, String[] values) throws CommonException {
        ...
        for (int n = 0; n < cols.length; n++) {
            ...
            if (!attrName.equals(StringUtils.strCat(DBConstants.SQLConstants.SQL_COLON, attrN, DBConstants.SQLConstants.SQL_COLON))) {
